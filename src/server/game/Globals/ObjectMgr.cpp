...
void ObjectMgr::LoadGraveyardZones()
{
    ...
    do
    {
        ...
        if (areaEntry->zone != 0 && zoneId != 33 && zoneId != 5287)
        {
...
void ObjectMgr::LoadRewardOnKill()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 creature reward reputation definitions. DB table `creature_onkill_reward` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (rewOnKill.repfaction2)
        {
            ...
            if (!factionEntry2)
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "Faction (faction.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.repfaction2);
                continue;
            }
        }
        ...
        if (rewOnKill.currencyid1)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "CurrenctType (CurrenctTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.currencyid1);
                continue;
            }
        }
        ...
        if (rewOnKill.currencyid2)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "CurrenctType (CurrenctTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.currencyid2);
                continue;
            }
        }
        ...
        if (rewOnKill.currencyid3)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "CurrenctType (CurrenctTypes.dbc) %u does not exist but is used in `creature_onkill_reward`", rewOnKill.currencyid3);
                continue;
            }
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u creature award reputation definitions", count);
}
...
const char *ObjectMgr::GetTrinityString(int32 entry, LocaleConstant locale_idx) const
{
    ...
    if (entry > 0)
        TC_LOG_ERROR(LOG_FILTER_SQL, "Entry %i not found in `arkcore_string` table.", entry);
    else

...
bool Unit::HasNegativeAuraWithAttribute(uint32 flag, uint64 guid)	bool Unit::HasAuraWithMechanic(uint32 mechanicMask)
{
...
void Unit::ReadMovementInfo(WorldPacket& data, MovementInfo* mi, Movement::ExtraMovementStatusElement* extras /*= NULL*/)
{
    ...
    if (sequence == NULL)
    {
        TC_LOG_ERROR(LOG_FILTER_NETWORKIO, "Unit::ReadMovementInfo: No movement sequence found for opcode %s", GetOpcodeNameForLogging(data.GetOpcode()).c_str());
        return;
    }
   ...
   if (hasTransportData && mi->pos.m_positionX != mi->t_pos.m_positionX)
    ...
    { \
        if (check) \
        { \
            TC_LOG_DEBUG(LOG_FILTER_UNITS, "Unit::ReadMovementInfo: Violation of MovementFlags found (%s). " \

        ...
        if (player->GetQuestStatus(12701) == QUEST_STATUS_INCOMPLETE)
        {
            ...
            if (Creature* miner = player->SummonCreature(28841, 2383.869629f, -5900.312500f, 107.996086f, player->GetOrientation(), TEMPSUMMON_DEAD_DESPAWN, 1))
            {
                ...
                if (Creature* car = player->GetVehicleCreatureBase())
                {
                    ...
                    } else TC_LOG_ERROR(LOG_FILTER_TSCR, "OnGossipHello vehicle entry is not correct.");
                } else TC_LOG_ERROR(LOG_FILTER_TSCR, "OnGossipHello player is not on the vehicle.");
            } else TC_LOG_ERROR(LOG_FILTER_TSCR, "OnGossipHello Scarlet Miner cant be found by script.");
        }
        return true;
    }
